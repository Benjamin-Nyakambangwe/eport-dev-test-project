...
const DataCollectionScreen = () => {
  ...
  const loadReferenceData = async () => {
    ...
    try {
      ...
      if (isNetworkAvailable) {
        ...
        console.log(
          `Loaded ${farmTypesResponse.data.length} farm types and ${cropsResponse.data.length} crops from API`
        );
      } else {
        ...
        if (localFarmTypes && localFarmTypes.length > 0) {
          ...
          Alert.alert(
            "Offline Mode",
            "No farm types found in local database. Add test data or sync when online."
          );
        }
        ...
        if (localCrops && localCrops.length > 0) {
          ...
          Alert.alert(
            "Offline Mode",
            "No crops found in local database. Add test data or sync when online."
          );
        }
      }
    } catch (error) {
  ...
  const validateForm = () => {
    ...
    if (!formData.farm_type_id)
      newErrors.farm_type_id = "Farm type is required";
  ...
  return (
    ...
    >
      ...
      >
        ...
        <View style={styles.formGroup}>
          <Text style={styles.label}>
            National ID <Text style={styles.required}>*</Text>
          </Text>
          <TextInput
        ...
        <View style={styles.formGroup}>
          <Text style={styles.label}>
            Farm Type <Text style={styles.required}>*</Text>
          </Text>
          <View
            style={[
              styles.pickerContainer,
              errors.farm_type_id && styles.pickerError,
            ]}
          >
            <Picker
              selectedValue={formData.farm_type_id}
              onValueChange={(value) =>
                handleInputChange("farm_type_id", value)
              }
              style={styles.picker}
            >
          ...
          {errors.farm_type_id && (
            <Text style={styles.errorText}>{errors.farm_type_id}</Text>
          )}
        </View>
        ...
        <View style={styles.formGroup}>
          ...
          <View
            style={[
              styles.pickerContainer,
              errors.crop_id && styles.pickerError,
            ]}
          >
            <Picker
          ...
          {errors.crop_id && (
            <Text style={styles.errorText}>{errors.crop_id}</Text>
          )}
        </View>
